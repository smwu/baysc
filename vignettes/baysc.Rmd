...
The `wolca()` function ([Section 2.3.1](#wolca)) implements a survey-weighted Bayesian LCA and can be followed by `wolca_var_adjust()` for correct uncertainty estimation. Options also exist for relating the pattern profiles to a binary outcome. This can either be done through a two-step approach after clustering is completed using the `wolca_svyglm()` function described in [Section 2.3.1.3](#wolca_svyglm)), or in a one-step supervised approach where creation of the pattern profiles is directly informed by the outcome using the `swolca()` function described in [Section 2.3.2](#swolca), followed by `swolca_var_adjust()` for correct uncertainty estimation. The two-step approach is advantageous for relating the same set of patterns to a variety of different outcomes and is computationally faster. The one-step approach is advantageous for propagating classification measurement error, detecting smaller exposure-outcome effects, and discovering patterns where a small subset of foods are highly influential for the outcome. Summary and plotting functions for visualizing output are available, as are diagnostic functions for examining convergence of the MCMC sampler (Sections [2.4](#plot) and [2.5](#diagnostics)). Full statistical details for the models are provided in @wu2024derivation.
...
::: figure
<img src="lca_diagram.png" alt="Figure: Diagram of latent class analysis" width="100%"/>

<p class="caption">

Figure: Diagram of latent class analysis
...
Interest may also lie in examining the relationship between the latent class pattern profiles and a binary outcome. This can be done in two ways: 1) a two-step "classify-analyze" approach that performs a regression after clustering is completed ([Section 2.3.1.3](#wolca_svyglm)), or 2) a one-step approach where creation of the pattern profiles is directly informed by the outcome ([Section 2.3.2](#swolca)).
...
The `baysc` package can be installed and loaded from GitHub by running the following code.
...
### Define categorical item variables

We define the matrix of categorical variables that will be used for the clustering analysis. In this example, we have a sample size of $n=2004$ and consider $J=28$ food groups, each of which is a categorical variables with four levels of intake: 1, 2, 3, 4.
...
### Define outcome additional regression variables
...
### Option 1: Use WOLCA to derive dietary exposure clusters {#wolca}

#### Clustering analysis with `wolca()`

We use the `wolca()` function to perform unsupervised clustering analysis and obtain pattern profiles. We run the sampler for 300 iterations as an example. Typically, at least 20000 iterations should be used for `wolca()`, corresponding to a runtime of around 2 hours.
...
-   `adapt_seed = 111`: Random seed for adaptive sampler, set to 111 for this example but with default `NULL`.
...
#### Variance adjustment with `wolca_var_adjust()`
...
-   `adjust_seed = 111`: Random seed for the adjustment procedure, set to 111 for this example but with default `NULL`.
...
### Option 2: Use SWOLCA to derive dietary exposure clusters informed by a binary hypertension outcome {#swolca}

#### Clustering analysis and outcome association with `swolca()`

As an alternative to the two-step approach in [Section 2.3.1.3](#wolca_svyglm), a one-step “supervised” approach allows information about the binary outcome to directly inform the creation of the pattern profiles. We use the `swolca()` function to obtain dietary pattern profiles while simultaneously relating them to the hypertension outcome. 
We run the sampler for 100 iterations as an example. Typically, at least 20000 iterations should be used for `swolca()`, corresponding to a runtime of around 3 hours.
...
#### Variance adjustment with `swolca_var_adjust()`

The `swolca_var_adjust()` function is used after to apply a post-processing adjustment that allows for correct estimation of the variability of the parameter estimates. If this adjustment is not applied, the estimated variances will be too small. 
...
<img src="figure/plot_pattern_profiles-1.png" alt="Figure: Latent class pattern profiles"/>

<p class="caption">

Figure: Latent class pattern profiles

</p>
:::

We can also specify labels for the (`item_labels`), categories (`categ_labels`), and latent classes (`class_labels`), as well as the axis and legend titles (`item_title`, `categ_title`, and `class_title`).
...
::: {.figure style="text-align: center"}
<img src="figure/profiles_labels-1.png" alt="Figure: Latent class pattern profiles with custom labels"/>

<p class="caption">

Figure: Latent class pattern profiles with custom labels
...
::: {.figure style="text-align: center"}
<img src="figure/plot_pattern_probs-1.png" alt="Figure: Item probabilities for each latent class pattern"/>

<p class="caption">

Figure: Item probabilities for each latent class pattern
...
In this example, we can see that individual 1 is assigned to class 1 and individual 2 is assigned to class 2 with high probabilities (\>0.95), and individual 3 is assigned to class 2 with lower probability (0.85).
...
```         
##       [,1]  [,2]  [,3] [,4] [,5]
## [1,] 0.999 0.000 0.001    0    0
## [2,] 0.000 0.958 0.042    0    0
## [3,] 0.001 0.147 0.851    0    0
```
...
::: {.figure style="text-align: center"}
<img src="figure/dendrogram-1.png" alt="Figure: Dendrogram of class separation"/>

<p class="caption">

Figure: Dendrogram of class separation
...
::: {.figure style="text-align: center"}
<img src="figure/reorder_classes-1.png" alt="Figure: Latent class pattern profiles with reordered classes"/>

<p class="caption">

Figure: Latent class pattern profiles with reordered classes
...
::: {.figure style="text-align: center"}
<img src="figure/plot_class_dist-1.png" alt="Figure: Distribution of latent classes in the population"/>

<p class="caption">

Figure: Distribution of latent classes in the population
...
```         
## [1] 0.251 0.216 0.188 0.195 0.151
```
...
| Variable | Level | C1 | C2 | C3 | C4 | C5 | Overall |
|:--------|:--------|:--------|:--------|:--------|:--------|:--------|:--------|
| N: % (posterior SD %) |  | 23.8 (5.9) | 21.1 (8.1) | 18.9 (13.4) | 18.1 (5.1) | 18.0 (16.6) |  |
| n: % |  | 25.2 | 23.1 | 13.3 | 21.5 | 17.0 |  |
| Age: Mean |  | 44.0 | 43.8 | 47.0 | 50.1 | 47.4 | 46.2 |
| Age_Group: % | [20, 40) | 46.3 | 49.5 | 37.8 | 31.1 | 41.9 | 41.9 |
|  | [40, 60) | 30.7 | 26.7 | 38.6 | 40.5 | 27.4 | 32.6 |
|  | \>=60 | 23.0 | 23.8 | 23.5 | 28.5 | 30.7 | 25.5 |
| Race_Ethnicity: % | NH White | 51.1 | 44.1 | 64.0 | 46.7 | 41.9 | 49.3 |
|  | NH Black | 17.6 | 16.4 | 10.7 | 15.5 | 20.8 | 16.3 |
|  | NH Asian | 3.1 | 5.2 | 2.3 | 7.5 | 11.4 | 5.6 |
|  | Hispanic/Latino | 23.0 | 28.7 | 20.6 | 21.8 | 22.7 | 23.6 |
|  | Other/Mixed | 5.1 | 5.6 | 2.3 | 8.5 | 3.2 | 5.2 |
| Current_Smoker: % | No | 74.4 | 78.4 | 64.5 | 71.1 | 77.3 | 73.5 |
|  | Yes | 25.6 | 21.6 | 35.5 | 28.9 | 22.7 | 26.5 |
| Physical_Activity: % | Inactive | 45.5 | 44.5 | 44.4 | 47.1 | 42.9 | 45.1 |
|  | Active | 54.5 | 55.5 | 55.6 | 52.9 | 57.1 | 54.9 |
...
## 1  (Intercept)    -1.13 -1.48 -0.72     <0.01
## 2       c_all2    -0.10 -1.19  1.24      0.42
## 3       c_all3     0.08 -0.75  1.26      0.62
## 4       c_all4     0.28 -0.57  1.09      0.66
## 5       c_all5     0.24 -2.36  1.76      0.62
## 6     age_cat2     1.09  0.82  1.38      1.00
## 7     age_cat3     2.13  1.47  2.58      1.00
## 8   racethnic2     0.44 -0.13  1.08      0.94
## 9   racethnic3     0.06 -0.59  1.12      0.56
## 10  racethnic4    -0.16 -0.48  0.20      0.20
```
...
::: {.figure style="text-align: center"}
<img src="figure/plot_regr_coefs-1.png" alt="Figure: Outcome regression coefficients "/>

<p class="caption">

Figure: Outcome regression coefficients
...
::: {.figure style="text-align: center"}
<img src="figure/coefs_labels-1.png" alt="Figure: Outcome regression coefficients with custom labels"/>

<p class="caption">

Figure: Outcome regression coefficients with custom labels
...
::: {.figure style="text-align: center"}
<img src="figure/plot_outcome_probs-1.png" alt="Figure: Outcome probabilities for levels of a categorical covariate"/>

<p class="caption">

Figure: Outcome probabilities for levels of a categorical covariate
...
::: {.figure style="text-align: center"}
<img src="figure/probs_two-1.png" alt="Figure: Outcome probabilities for levels of two categorical covariates"/>

<p class="caption">

Figure: Outcome probabilities for levels of two categorical covariates
...
```         
## Time difference of 90.40523 secs
```
...
## 1      pi_1     0.25        0.10        0.33
## 2      pi_2     0.22        0.03        0.34
## 3      pi_3     0.19        0.02        0.47
```
...
```         
##     Parameter Estimate Lower Bound Upper Bound
## 6 theta_1_1_1     0.65        0.43        0.76
## 7 theta_2_1_1     0.46        0.29        0.59
## 8 theta_3_1_1     0.44        0.38        0.52
## 9 theta_4_1_1     0.58        0.41        0.70
```
...
```         
##     Parameter Estimate Lower Bound Upper Bound
## 566    xi_1_1    -1.13       -1.48       -0.72
## 567    xi_2_1    -1.24       -2.27        0.15
## 568    xi_3_1    -1.02       -1.78       -0.15
## 569    xi_4_1    -0.86       -1.54       -0.21
```
...
## 1  0.199337680 0.29798426 0.1279449684 0.2206540 0.15407908
## 2  0.256967542 0.30446362 0.2021383390 0.1572870 0.07914354
## 3  0.263425322 0.08413522 0.4648721743 0.1231027 0.06446456
## 4  0.221394758 0.23644027 0.1722512104 0.2167745 0.15313927
## 5  0.269240234 0.18652288 0.2918798644 0.1791225 0.07323449
## 6  0.311260177 0.19111240 0.2805098911 0.1457299 0.07138768
## 7  0.008695988 0.01466584 0.0003048905 0.0195115 0.95682179
## 8  0.196751600 0.24555358 0.0486972000 0.2042066 0.30479104
## 9  0.277954303 0.22683991 0.1138511948 0.2062676 0.17508698
## 10 0.245779276 0.27931692 0.0645889321 0.1847761 0.22553878
```
...
::: {.figure style="text-align: center"}
<img src="figure/traceplots-1.png" alt="Figure: Traceplots for pi"/>

<p class="caption">

Figure: Traceplots for pi
...
::: {.figure style="text-align: center"}
<img src="figure/acf_plots-1.png" alt="Figure: ACF plots for pi"/>

<p class="caption">

Figure: ACF plots for pi
